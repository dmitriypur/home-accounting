      ...
      <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          <div>
            ...
            <p class="mt-2 text-gray-600">Управление вашими доходами</p>
          </div>
          <button 
            ...
            >
              <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                <div class="flex items-center space-x-4 min-w-0 flex-1">
                  <div class="w-12 h-12 bg-gradient-to-br from-green-50 to-blue-50 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-200 flex-shrink-0">
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-1 sm:space-y-0 text-sm text-gray-500">
                      <span class="flex items-center">
                        ...
                        <span class="truncate">{{ income.category.name }}</span>
                      </span>
                      <span class="flex items-center">
                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center space-x-3">
                  <div class="text-left sm:text-right flex-shrink-0">
                    <p class="text-lg font-bold text-green-600 mb-1">+{{ formatCurrency(income.amount) }}</p>
          ...
          <form @submit.prevent="saveIncome">
            ...
            <div class="flex justify-end space-x-3 mt-6">
              ...
              <button type="submit" :disabled="saving" class="btn btn-primary">
                {{ saving ? 'Сохранение...' : (editingIncome ? 'Обновить' : 'Добавить') }}
              </button>
            </div>
          </form>
        </div>
  ...
  setup() {
    ...
    const loadIncomes = async (page = 1) => {
      ...
      try {
        ...
        alert('Ошибка при загрузке доходов')
      } finally {
        loading.value = false
      }
    }
    ...
    const saveIncome = async () => {
      ...
      try {
        ...
        closeModal()
        loadIncomes(pagination.value.current_page)
      } catch (error) {
